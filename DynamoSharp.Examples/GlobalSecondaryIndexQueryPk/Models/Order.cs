using GlobalSecondaryIndexQueryPk.Exceptions;
    ...
    public class Builder
    {
        ...
        public void Validate()
        {
            if (_order.Id == Guid.Empty)
                throw new ArgumentException("Id cannot be empty", nameof(_order.Id));

            if (_order.BuyerId == Guid.Empty)
                throw new ArgumentException("BuyerId cannot be empty", nameof(_order.BuyerId));

            if (_order.Address == null)
                throw new ArgumentException("Address cannot be null", nameof(_order.Address));

            if (_order.Date == DateTime.MinValue)
                throw new ArgumentException("Date cannot be empty", nameof(_order.Date));
        }
    }
}
